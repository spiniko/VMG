javascript:(function() {
    window.generateMessage = function() {
        ...
        const os = 
            /Windows NT 10.0/.test(userAgent) ? "Windows 10" :
        ...
        const { browserName, fullVersion } = 
            /Firefox\/(\S+)/.exec(userAgent) ? { browserName: "Firefox", fullVersion: /Firefox\/(\S+)/.exec(userAgent)[1] } :
            /Chrome\/(\S+)/.exec(userAgent) && !/Edg/.exec(userAgent) ? { browserName: "Chrome", fullVersion: /Chrome\/(\S+)/.exec(userAgent)[1] } :
            /Safari\/(\S+)/.exec(userAgent) && !/Chrome/.exec(userAgent) ? { browserName: "Safari", fullVersion: /Version\/(\S+)/.exec(userAgent)[1] } :
            { browserName: "Unknown Browser", fullVersion: "X.X.X" };
        ...
        const finalStatus = document.querySelector("#dropdownBasic1").textContent.trim();
        ...
        const screenshotName = `Screenshot (attached): ${issueKey}_${formattedDate}_${finalStatus.split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join('_')}_Validation.png`;
        const testNotes = `Notes: The issue initially reported is ${status}.`;

        let alertMsg, msgTemplate, isValidationCompleted, recommendation, showRecommendation;
        isValidationCompleted = status !== statuses.open &&  status !== statuses.toReview;
        showRecommendation = status === statuses.notFixed || status === statuses.partiallyFixed; 
        recommendation = showRecommendation ? "Recommendation: We recommended to follow the original guidance." : "";
        alertMsg = isValidationCompleted ? `The Validation Message has been generated.\n\nPlease review the Comments and Activity section and make any necessary edits before posting.` : `Please complete the validation and update the issue STATUS before proceeding.`; 
        msgTemplate = "";

        if (isValidationCompleted) {
            msgTemplate = `${testLocation}\n${testEnvironment}\n${testTools}\n${testStatus}\n${screenshotName}\n${testNotes}\n${recommendation}`;
            msgBox.value = msgTemplate.trim(), msgBox.style.height = '250px', msgBox.focus();
        }
